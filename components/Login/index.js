...
class Login extends React.Component {
  ...
  google(evt) {
    ...
    if (!gapi.auth2) {
      console.log('not');
      p = new Promise(function (resolve, rej) {
        gapi.load('auth2', function () {
          //client_secret p6g60R0ChcUT7WIc1apC386k
          gapi.auth2.init({
            client_id: '892140925767-dsjrr4igp1ngm4oci2k1penrjl3lf3oe.apps.googleusercontent.com'
          }).then(function () {
            resolve();
          })
    ...
    p.then(function () {
      ...
      }).then(function (onSignin) {
        const email = gapi.auth2.getAuthInstance().currentUser.get().getBasicProfile().getEmail();
        that.setState({email:email})
      })
    })
  }
  ...
  render() {
    return (
      <Box style={_extends({},this.props.style,{flexFlow:'row nowrap',alignItems:'center' })}>
        ...
        <div className={s.google}  onClick={this.google.bind(this) }>
          <svg
            className={s.icon}
            width="30"
            height="30"
